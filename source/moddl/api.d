...
struct CitserverAPI {
  ...
  public auto versions(string id) {
    return getIndexNode[id].object.byKey.filter!(a => a.isMatch(ctRegex!(`^[0-9\.]+$`)));
  }

  public string latestVersion(string id) {
    return this.versions(id).maxCount!((a, b) => cmpVersion(a, b) < 0)()[0];
  }
  ...
  public auto fileList(string id, string ver) {
    ...
    return std.net.curl.get([this.apiRoot, id, ver ~ ".json"].join("/")).parseJSON()["files"].array;
  }

  private JSONValue getIndexNode() {
    return std.net.curl.get(this.apiRoot ~ "/index.json").parseJSON();
  }
}
